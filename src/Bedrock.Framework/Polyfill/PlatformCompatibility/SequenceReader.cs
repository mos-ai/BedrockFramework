...
namespace System.Buffers
{
    public ref partial struct SequenceReader<T> where T : unmanaged, IEquatable<T>
    {
        ...
        public SequenceReader(ReadOnlySequence<T> sequence)
        {
            ...
            // TODO: Validate and check. No idea if this is valid or going to work but hey, first pass :P

            //sequence.GetFirstSpan(out ReadOnlySpan<T> first, out _nextPosition);
            var first = sequence.First.Span;
            _nextPosition = sequence.GetPosition(first.Length + 1);
        ...
        {
            if (offset < 0)
                throw new ArgumentOutOfRangeException("OffsetOutOfRange");
